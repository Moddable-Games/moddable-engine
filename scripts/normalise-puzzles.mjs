...
// The file held one licence sentence over pools that are licensed differently,
// so a consumer could not take the CC0 records alone (engine#178 C3). Each
// record now carries its source's licence, and `meta.licenses` says where each
// one comes from. A source with no entry here stops the run rather than being
// given a licence nobody checked.
const LICENSES = {
  'lichess-cc0': {
    license: 'CC0-1.0',
    basis: 'database.lichess.org: "Database exports are released under the Creative Commons CC0 license" (checked 2026-09-24)',
  },
  historical: {
    license: 'public-domain',
    basis: "classical positions and studies, the latest Réti's study of 1921 (Réti died in 1929), as the pool has always declared them",
  },
  'engine-generated': {
    license: 'proprietary',
    basis: "generated by this engine's own puzzle generator, as the pool has always declared it",
  },
  'polgar-5334': {
    license: 'unverified',
    basis: 'taken from Laszlo Polgar, Chess: 5334 Problems, Combinations and Games (1994). No licence for this use is recorded; treat as not cleared',
  },
}

function licenseFor(record) {
  const entry = LICENSES[record.source]
  if (!entry) {
    console.error(`Record ${record.id} has source "${record.source}", which has no licence entry in scripts/normalise-puzzles.mjs. Refusing to guess.`)
    process.exit(1)
  }
...
function normalise(data) {
  ...
  function normaliseRecord(record, defaultVariant) {
    ...
    // shogi and draughts.
    return { ...record, family, position, turn: turnOf(position), variantSlug: slug, license }
  }
  ...
  const normalised = {
    ...
    meta: {
      ...
      schemaVersion: 2,
      normalised: DATE,
      license: 'Per record: each record carries its own `license`, and `licenses` gives the basis for each source',
      licenses: Object.fromEntries(Object.entries(LICENSES).map(([source, entry]) => [source, { ...entry }])),
    },
