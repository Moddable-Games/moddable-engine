function abilitiesOf(type) {
  if (type === 'queen' || type === 'amazon') return 7
  if (type === 'chancellor') return 5
  if (type === 'rook') return 4
  if (type === 'archbishop') return 3
...
function typeForAbilities(ab) {
  if (ab >= 7) return 'amazon'
  if (ab === 6) return 'queen'
  if (ab === 5) return 'chancellor'
  if (ab === 4) return 'rook'
  if (ab === 3) return 'archbishop'
