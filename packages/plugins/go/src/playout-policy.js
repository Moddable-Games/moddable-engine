...
// `random` is injected rather than taken from the ambient generator, so a
// rollout is reproducible from a seed. This file used to live in packages/ai,
// outside the reach of the no-ambient-random guard, and MCTS was quietly
// non-deterministic no matter what rngSeed the caller passed. When no
// generator is supplied it seeds its own, so a policy is still reproducible
// within a single instance rather than drawing from global state.
export function createGoPlayoutPolicy(random) {
  const rng = random || (() => { const r = createRng(1); return () => r.next() })()
