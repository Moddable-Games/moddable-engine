...
export function createGraphTopology(config) {
  ...
  // Renders explicit nodes/edges, or delegates to the op pipeline. It does NOT
  // generate structures from `params` — named structures (concentric-rings,
  // perimeter-cross, grid-cross, star) are built once, parametrically, in
  // packages/schema/src/produce-layout.js and arrive here as ops. A second
  // generator here drifted from that one (star lost its rim and board
  // furniture, grid-cross ignored extraNodes/fortressExtraRow, perimeter-cross
  // numbered its diagonal nodes differently) while being unreachable; #130
  // deleted it. Add structure geometry to produce-layout, never here.
  function renderLayout(config = {}) {
    ...
    const zones = inputZones
    const structures = inputStructures
