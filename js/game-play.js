...
export function createPlaySession(options = {}) {
  ...
  async function start() {
    ...
    let humanIdx
    if (seat === '0' && names.length > 2) {
      const state = game.getState()
      humanIdx = defaultSeatFor(
        state?.slice?.board || [],
        resolvedBoard.topology?.cols || 0,
        names.length,
        resolvedBoard.defaultSeat,
      )
    } else {
      humanIdx = resolveHumanIndex(seat, names)
    }
    const players = {}
    ...
    ctrl = createGameController(game.raw, {
      family,
      players,
      interactionModel: resolvedBoard.interaction || resolvedBoard.interactionModel,
