...
import { getDeckConfig, createDeck, shuffle, deal, layoutTable } from './deck-manager/index.js'
...
export async function loadGalleryIndex(basePath = '../pieces/') {
  if (galleryIndex) return galleryIndex
  try {
    const resp = await fetch(basePath + 'gallery-index.json')
...
export async function renderFromResolved(resolved, container) {
  const topo = resolved.topology || {}

  // Non-spatial games — pipe through deck-manager + renderDeckSvg
  if (topo.type === 'none' || !topo.type) {
    ...
    // Fallback for RPG or games without deck/dice
    container.innerHTML = `<div style="padding:20px;color:#aaa;text-align:center;font-family:system-ui">
      <p style="font-size:16px;margin-bottom:8px">${meta.label || 'Non-spatial game'}</p>
      <p style="font-size:12px;color:#666">Category: ${meta.category || 'unknown'}</p>
    </div>`
    return
  }
  ...
  if (!opts) {
    container.innerHTML = '<p style="color:#f44">Cannot determine board style from resolved definition</p>'
    return
  }

  // Load gallery and attach piece images
...
export { resolveBoardStyle, mapColorsForProvider, buildRenderOpts }
