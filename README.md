...
- Gygax Chess (Dragonchess) plays: three boards, fifteen piece types, each moving differently on each board. It needed four things that are now general. A capture from afar that leaves the capturer where it stands (the Dragon), a freeze declared as a piece type and an offset (the Basilisk), a quiet move to any empty cell of a region (the Sylph's return home), and a leap that is blocked by the cell on its level or the cell on its column (the Elemental)
